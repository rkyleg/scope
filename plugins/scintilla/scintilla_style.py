# Color is a rgb string

#---Default Theme
styleD = {
    # Python
    'Default':'255,255,255',  #white
##    'Default':'166,220,255',  # blue
##    'Default':'121,193,255',  # blue (good)
##    'Default':'67,183,255',  # blue
##    'Default':'150,220,255',  # blue

##    'Comment':'150,150,150', # gray
##    'Comment':'250,255,151', # yellow
    'Comment':'244,255,166', # yellow
    'CommentBlock':'100,100,100',
    
##    'ClassName':'0,170,255', # blue
    'ClassName':'255,116,88', # orange

##    'ClassName':'62,115,156', # light blue
    'FunctionMethodName':'255,201,139',   # orange
##    'FunctionMethodName':'97,171,205',   # blue
##    'FunctionMethodName':'198,140,255',   # purple
    'GlobalClass':'255,177,99', # orange, javascript
    'Tag':'255,177,99', # orange, html
    
    'Decorator':'255,170,127',
##    'Keyword':'255,177,99', # orange
    'Keyword':'97,171,205', # light blue
##    'Keyword':'198,140,255', # purple
##    'Keyword':'40,97,167', # blue
##    'Keyword':'125,188,151',    # green
##    'FunctionMethodName':'92,164,219',   # blue
    
    'HighlightedIdentifier':'250,0,0',
    'Identifier':'',
    'Inconsistent':'',
    'NoWarning':'',
    
##    'Operator':'254,255,172',  # yellow
    'Operator':'166,220,255',  # blue
##    'Operator':'255,255,255',  # white
    
    'Spaces':'',
    'Tabs':'',
    'TabsAfterSpaces':'',
    
##    'Number':'125,255,125', # green
    'Number':'255,170,255', # pink
    
    'SingleQuotedString':'125,188,151',
    'DoubleQuotedString':'125,188,151',
    'TripleDoubleQuotedString':'122,159,94',
    'TripleSingleQuotedString':'122,159,94',
##    'UnclosedString':'255,255,127', # yellow
    'UnclosedString':'255,94,94', # reddish
    
    
    # Other Javascript
##    'CommentLine':'244,255,166', # yellow
##    'CommentDoc':'244,255,166', # yellow
####    'CommentLine':'100,100,100', # yellow
####    'CommentDoc':'100,100,100', # yellow
##    'InactiveComment':'100,100,100',
##    'InactiveCommentDoc':'100,100,100',
##    'InactiveComment':'100,100,100',
##    'CommentDocKeyword':'244,255,166', # yellow
##    'InactiveCommentDocKeyword':'244,255,166', # yellow
##    'Regex':'170,0,255',
}


#---Light Theme
styleLightD = {
    # Python
    'Default':'0,0,0',  #black
##    'Default':'166,220,255',  # blue
##    'Default':'121,193,255',  # blue (good)
##    'Default':'67,183,255',  # blue
##    'Default':'150,220,255',  # blue

##    'Comment':'150,150,150', # gray
##    'Comment':'250,255,151', # yellow
    'Comment':'100,16,66', # brown
    'CommentBlock':'150,150,150',
    
##    'ClassName':'0,170,255', # blue
    'ClassName':'255,46,46', # orange

##    'ClassName':'62,115,156', # light blue
    'FunctionMethodName':'255,46,46',   # orange
##    'FunctionMethodName':'97,171,205',   # blue
##    'FunctionMethodName':'198,140,255',   # purple
    'GlobalClass':'255,46,46', # orange, javascript
    'Tag':'255,46,46', # orange, html
    
    'Decorator':'255,170,127',
##    'Keyword':'255,177,99', # orange
    'Keyword':'38,57,141', # light blue
##    'Keyword':'198,140,255', # purple
##    'Keyword':'40,97,167', # blue
##    'Keyword':'125,188,151',    # green
##    'FunctionMethodName':'92,164,219',   # blue
    
    'HighlightedIdentifier':'250,0,0',
    'Identifier':'',
    'Inconsistent':'',
    'NoWarning':'',
    
##    'Operator':'254,255,172',  # yellow
    'Operator':'61,86,138',  # blue
##    'Operator':'255,255,255',  # white
    
    'Spaces':'',
    'Tabs':'',
    'TabsAfterSpaces':'',
    
##    'Number':'125,255,125', # green
    'Number':'142,78,167', # pink
    
    'SingleQuotedString':'61,109,69',
    'DoubleQuotedString':'61,109,69',
    'TripleDoubleQuotedString':'80,104,61',
    'TripleSingleQuotedString':'80,104,61',
##    'UnclosedString':'255,255,127', # yellow
    'UnclosedString':'255,94,94', # reddish
    
    
    # Other Javascript
##    'CommentLine':'244,255,166', # yellow
##    'CommentDoc':'244,255,166', # yellow
####    'CommentLine':'100,100,100', # yellow
####    'CommentDoc':'100,100,100', # yellow
##    'InactiveComment':'100,100,100',
##    'InactiveCommentDoc':'100,100,100',
##    'InactiveComment':'100,100,100',
##    'CommentDocKeyword':'244,255,166', # yellow
##    'InactiveCommentDocKeyword':'244,255,166', # yellow
##    'Regex':'170,0,255',
}